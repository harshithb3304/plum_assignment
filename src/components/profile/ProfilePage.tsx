...
export function ProfilePage() {
  ...
  useEffect(() => {
    ...
    if (existingProfile) {
      ...
      setSelectedGoals(existingProfile.goals.filter(goal => WELLNESS_GOALS.includes(goal as any)));
      setCustomGoals(existingProfile.goals.filter(goal => !WELLNESS_GOALS.includes(goal as any)));
      
      // Show custom input if there are custom goals or if "Others" was selected
      if (existingProfile.goals.some(goal => !WELLNESS_GOALS.includes(goal as any)) || 
          existingProfile.goals.includes('Others')) {
        setShowCustomInput(true);
        setSelectedGoals(prev => prev.includes('Others') ? prev : [...prev, 'Others']);
      }
    }
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    const allGoals = [...selectedGoals.filter(g => g !== 'Others'), ...customGoals];
    ...
    try {
      ...
      // Simulate progress similar to TipsScreen
      let progress = 0;
      const progressInterval = setInterval(() => {
        progress = Math.min(95, progress + Math.random() * 12);
        // We can't set local state here easily; instead we rely on the overlay to show indeterminate progress.
      }, 200);
  ...
  <div className="relative z-10 flex flex-col flex-1">
        <header className="px-4 sm:px-6 py-4 sm:py-6">
          <div className="container mx-auto flex justify-between items-center">
            <Link href="/" className="flex items-center space-x-2 cursor-pointer hover:scale-105 transition-transform duration-300">
              ...
              <span className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white tracking-tight">Wellness AI</span>
            </Link>
            
            <div className="flex items-center gap-2 sm:gap-4">
              <ThemeToggle />
              <Link href="/" className="cursor-pointer">
                <Button variant="ghost" size="sm" className="cursor-pointer hover:scale-105 transition-transform duration-300 hover:bg-accent dark:hover:bg-accent/50">
          ...
          <Card className="shadow-2xl border-0 bg-white/50 dark:bg-white/5 backdrop-blur-sm">
            <CardHeader className="text-center pb-8">
              ...
              <CardDescription className="text-base sm:text-lg text-gray-700 dark:text-gray-400 mt-2">
                Tell us about yourself to get personalized AI-powered wellness recommendations
              </CardDescription>
            </CardHeader>
            ...
            <CardContent className="space-y-8">
              {isSubmitting ? (
                <div className="space-y-6 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Creating Your Personalized Tips ✨</h3>
                  <p className="text-sm text-gray-700 dark:text-gray-400">AI is analyzing your profile and generating custom recommendations...</p>
                  <Progress value={50} className="w-full h-2 bg-muted/30 dark:bg-white/10" />
                </div>
              ) : (
              <div className="space-y-6">
              <div className="space-y-3">
                <Label htmlFor="age" className="text-sm font-medium text-foreground dark:text-white">Age *</Label>
                <Input
                  id="age"
                  type="number"
                  ...
                  className={`w-full cursor-pointer transition-all duration-200 focus:scale-[1.02] bg-background/50 dark:bg-white/5 border-muted dark:border-white/20 ${errors.age ? 'border-red-500' : ''}`}
                  min="13"
                  max="120"
                />
              ...
              <div className="space-y-3">
                <Label className="text-sm font-medium text-foreground dark:text-white">Gender *</Label>
                <Select value={gender} onValueChange={setGender}>
                  <SelectTrigger className={`w-full cursor-pointer transition-all duration-200 hover:scale-[1.02] bg-background/50 dark:bg-white/5 border-muted dark:border-white/20 ${errors.gender ? 'border-red-500' : ''}`}>
                    <SelectValue placeholder="Select your gender" />
                  </SelectTrigger>
                  <SelectContent className="bg-background dark:bg-black border-muted dark:border-white/20">
                    {GENDER_OPTIONS.map((option) => (
                      <SelectItem key={option} value={option} className="cursor-pointer hover:bg-muted dark:hover:bg-white/10 text-foreground dark:text-white">
                        {option}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div className="space-y-4">
                <Label className="text-sm font-medium text-foreground dark:text-white">Wellness Goals * (Select all that apply)</Label>
                ...
                {showCustomInput && (
                  <div className="space-y-3 p-4 bg-muted/30 dark:bg-white/5 rounded-lg border border-muted dark:border-white/10">
                    <Label className="text-sm font-medium text-foreground dark:text-white">Add Your Custom Goals</Label>
