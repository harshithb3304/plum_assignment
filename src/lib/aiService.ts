...
export async function generateWellnessTips(profile: UserProfile): Promise<WellnessTip[]> {
  ...
  try {
    ...
    const availableNames: string[] = list?.models?.map((m: any) => m.name) || [];
    if (availableNames.length > 0) {
      modelsToTry = modelsToTry.filter((m) => availableNames.includes(m));
    }
  } catch (err) {
    // If listing fails, continue with the preconfigured list
    console.warn('Could not list models, proceeding with default candidates', err);
  }

  const prompt = `Generate 5 personalized wellness tips for a ${profile.age}-year-old ${profile.gender} with goals: ${profile.goals.join(', ')}.
...
Make tips actionable, age-appropriate, and goal-specific. Keep them engaging and motivational.`;
  ...
  for (const modelName of modelsToTry) {
    ...
    try {
      ...
      console.warn(`Model ${modelName} failed:`, status || statusText || message);

      const isTransient = status === 503 || /Service Unavailable|overloaded|503/.test(statusText + message);
      if (isTransient) {
        // try next model
        continue;
      }
...
export async function generateDetailedTip(tip: WellnessTip, profile: UserProfile): Promise<WellnessTip> {
...
User Profile: ${profile.age}-year-old ${profile.gender}, Goals: ${profile.goals.join(', ')}
...
Format as JSON:
{
  ...
  for (const modelName of modelsToTry) {
    ...
    try {
      ...
      const statusText = error?.statusText || '';
      const message = error?.message || '';
      console.warn(`Model ${modelName} failed for detailed tip:`, status || statusText || message);
      const isTransient = status === 503 || /Service Unavailable|overloaded|503/.test(statusText + message);
      ...
      return {
        ...tip,
        longExplanation: `This ${tip.category} tip focuses on ${tip.shortDescription.toLowerCase()}. It's particularly beneficial for your goals and can be easily incorporated into your daily routine.`,
        steps: [
  ...
  console.error('All models failed for detailed tip; returning simple fallback details');
  return {
    ...tip,
    longExplanation: `This ${tip.category} tip focuses on ${tip.shortDescription.toLowerCase()}. It's particularly beneficial for your goals and can be easily incorporated into your daily routine.`,
    steps: [
